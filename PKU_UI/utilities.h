...
QVector<QString> search_Base (int numOfResult,cmp Compare, QString input, QVector<Cuisine> &cuisineList){
    // 创建一个临时向量来存储比较结果
    ...
    for (const Cuisine& cuisine : cuisineList) {
        temp.emplace_back(make_pair(cuisine, Compare(cuisine, input)));
    }
