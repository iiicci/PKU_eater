...
Compare1::Compare1(){}
float Compare1::operator()(Cuisine cuisine, QString input, int label) {
    std::random_device rd;  // Obtain a random number from hardware
    std::mt19937 gen(rd()); // Seed the generator
...
Compare3::Compare3(){}
float Compare3::operator()(Cuisine cuisine, QString str, int label) {
    int hot;
    hot=label/100;
    int spicy;
    spicy=(label-hot*100)/10;
    int place;
    place=(label-hot*100-spicy*10);
    const size_t len1 = cuisine.get_name().size(), len2 = str.size();
    vector<vector<unsigned int>> d(len1 + 1, vector<unsigned int>(len2 + 1));
    ...
    //将距离转换为匹配度
    double maxLen = max(cuisine.get_name().length(), str.length());
    ...
    {
        int dist = d[len1][len2];
        result=1.0 - double((double)dist / (double)maxLen);
    }

    if(hot==2)
    {
        if(cuisine.get_hot()==1)
            return -1;
    }
    else if(hot==3)
    {
        if(cuisine.get_hot()==0)
            return -1;
    }

    if(spicy==3)
    {
        if(cuisine.get_spicy()==1)
            return -1;
    }
    else if(spicy==2)
    {
        if(cuisine.get_spicy()==0)
            return -1;
    }

    if(place==2)
    {
        if(cuisine.get_place()==1)
            return -1;
    }
    else if(place==3)
    {
        if(cuisine.get_place()==0)
            return -1;
    }
    return result;
}
